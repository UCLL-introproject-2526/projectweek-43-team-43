...
class VideoScreen:
    def __init__(self, game):
        self.game = game
        # Lijst met beschikbare skins (zorg dat deze bestanden in je images/ map staan)
        self.available_skins = ["spaceshipp.png", "spaceship.png"]
        self.skin_index = self.available_skins.index(self.game.current_skin)

    def get_skin_text(self):
        # Maakt de tekst mooi: "Skin: Spaceshipp"
    ...
    def run(self, screen) -> GameState:
        ...
        # Maak de knop aan met de huidige skin naam
        skin_btn = UIElement((CENTER_X, 300), self.get_skin_text(), 35, YELLOW, "CHANGE_SKIN")
        
        # Je zou hier ook een achtergrond-knop kunnen toevoegen op dezelfde manier
        back_btn = UIElement((CENTER_X, 600), "Back to Options", 30, WHITE, GameState.OPTIONS)
        ...
        while True:
            ...
            self.game.draw_menu_background(screen)
            try:
                # Laad het huidige gekozen plaatje voor de preview
                preview_path = f"images/{self.game.current_skin}"
                preview_img = pygame.image.load(preview_path).convert_alpha()
                # Schaal hem lekker groot zodat je het verschil goed ziet
                ...
                # Fallback: als het plaatje mist, teken een gele cirkel
            ...
            for button in buttons:
                ...
                if ui_action == "CHANGE_SKIN":
                    # Ga naar de volgende skin in de lijst
                    ...
                    # Optioneel: speel een sfx als je wisselt
