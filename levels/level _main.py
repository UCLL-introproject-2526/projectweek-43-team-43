...
class LevelSelectScreen(MenuScreenBase):
    def build_buttons(self):
        title = UIElement((CENTER_X, 150), "SELECTEER LEVEL", 60, WHITE)
        
        
        lvl1 = UIElement((CENTER_X - 150, 350), "1", 80, GREEN, 1)
        lvl2 = UIElement((CENTER_X - 50, 350), "2", 80, YELLOW, 2)
        lvl3 = UIElement((CENTER_X + 50, 350), "3", 80, RED, 3)
        lvl4 = UIElement((CENTER_X + 150, 350), "4", 80, (255, 0, 255), 4) 
        
        back_btn = UIElement((CENTER_X, 550), "Terug naar Menu", 30, WHITE, GameState.TITLE)
    ...
    def run(self, screen) -> GameState:
        ...
        while True:
            result = self.game.game_loop(screen, buttons)
            
            
            if isinstance(result, int):
                self.setup_level_difficulty(result)
