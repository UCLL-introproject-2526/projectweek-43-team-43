...
try:
    ...
    print("Let op: Muziekbestand niet gevonden of mapstructuur is anders.")
...
def render_frame(surface, blocks, player_x, player_y, current_score, heart_img, current_lives, immunity_timer, background_img, img_small, img_medium, img_large, player_img):
    ...
    for block in blocks:
        ...
        if img_small and img_medium and img_large:
            ...
            # Fallback: Witte blokjes als plaatjes missen
...
def load_image_safe(filename):
    """ Probeert een plaatje te laden uit de map 'images/' """
    path = os.path.join("images", filename)
    try:
        img = pygame.image.load(path).convert_alpha()
        return img
    except (FileNotFoundError, pygame.error):
        print(f"FOUT: Kan afbeelding niet vinden: {path}")
        return None
...
def play_level(screen):
    ...
    try:
        ...
        print("FOUT: galaxy.png niet gevonden.")

    # AANGEPASTE BESTANDSNAAM: neptunus.png ipv meteoriet2.png
    meteor_small = load_image_safe("neptunus.png")
    meteor_medium = load_image_safe("mars.png")
    meteor_large = load_image_safe("jupiter.png")
    
    heart_image = load_image_safe("lives.png")
    if heart_image:
        heart_image = pygame.transform.scale(heart_image, (30, 30))

    # Player laden (als je die hebt, anders None)
    player_image_raw = load_image_safe("player.png")
    if player_image_raw:
        player_image = pygame.transform.scale(player_image_raw, (PLAYER_RADIUS * 2, PLAYER_RADIUS * 2))
    else:
        player_image = None
